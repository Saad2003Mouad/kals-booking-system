...
function Toast({
  ...
  return (
    <div
      className={`fixed top-5 right-5 z-50 flex items-center gap-3 px-5 py-3.5 rounded-2xl shadow-2xl text-sm font-bold border ${type === "success" ? "bg-emerald-50 border-emerald-200 text-emerald-800" : "bg-red-50 border-red-200 text-red-800"}`}
    >
      {type === "success" ? (
...
function AddZipModal({
  ...
  const handle = async () => {
    if (!/^\d{5}$/.test(zip)) {
      setErr("ZIP must be exactly 5 digits");
      return;
    }
    if (!city.trim()) {
    ...
    setErr("");
    try {
      await onAdd(zip, city, county, notes);
      onClose();
    } catch (e: any) {
      setErr(e.message || "Failed to add ZIP");
    } finally {
      setSaving(false);
    }
  };
  ...
  const inCls =
    "w-full py-3.5 px-5 rounded-2xl border-2 font-semibold text-base outline-none transition-all bg-white text-[#000223] border-slate-100 placeholder:text-slate-400 focus:border-[#FFA000] focus:ring-4 focus:ring-[#FFA000]/15";

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#000223]/40 backdrop-blur-sm">
      <div
        className="bg-white rounded-2xl shadow-xl w-full max-w-md mx-4 overflow-hidden border border-slate-200"
        style={{ fontFamily: FN }}
      >
        <div className="px-8 py-6 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
          <div>
            <h2 className="text-xl font-black" style={{ color: "#000223" }}>
              Add ZIP Code
            </h2>
            <p className="text-xs font-semibold text-[#000223]/50 mt-0.5">
              Add a Massachusetts service area ZIP
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="px-8 py-6 space-y-4 bg-white">
          {err && (
            <div className="flex items-center gap-2 text-red-600 bg-red-50 border border-red-100 rounded-2xl px-4 py-3 text-sm font-bold">
          ...
          <div>
            ...
            <input
              ...
              placeholder="Special notes or conditions"
              className={inCls}
            />
          </div>
        </div>
        <div className="px-8 py-5 border-t border-slate-100 bg-slate-50/50 flex justify-end gap-3">
          <button
            onClick={onClose}
            className="px-5 py-3 rounded-2xl text-sm font-extrabold border-2 border-slate-200 text-slate-500 hover:border-slate-400 bg-white transition-all"
          >
            Cancel
          </button>
          <button
            onClick={handle}
            disabled={saving}
            className="flex items-center gap-2 px-6 py-3 rounded-2xl text-sm font-black shadow-md hover:shadow-lg hover:-translate-y-0.5 transition-all disabled:opacity-50"
...
function EditModal({
  ...
  record: ZipRecord;
  onSave: (id: string, data: Partial<ZipRecord>) => Promise<void>;
  onClose: () => void;
}) {
      ...
      >
        ...
        <div className="px-8 py-5 border-t border-slate-100 bg-slate-50/50 flex justify-end gap-3">
          ...
          <button
            onClick={handle}
            disabled={saving}
            className="flex items-center gap-2 px-6 py-3 rounded-2xl text-sm font-black shadow-md hover:shadow-lg disabled:opacity-50 transition-all"
            style={{ background: "#000223", color: "#FFA000" }}
          >
            {saving ? (
              <Loader2 className="w-4 h-4 animate-spin" />
...
export default function ServiceAreasPage() {
  ...
  const [filterActive, setFilter] = useState<"all" | "active" | "inactive">(
    "all",
  );
  ...
  const handleToggle = async (rec: ZipRecord) => {
    ...
    setRecords((r) =>
      r.map((x) => (x.id === rec.id ? { ...x, isActive: !x.isActive } : x)),
    );
    showToast(
      "success",
      `${rec.zip} ${rec.isActive ? "deactivated" : "activated"}`,
    );
  };
  ...
  const handleBulkDeactivate = async () => {
    ...
    try {
      ...
      setRecords((r) =>
        r.map((x) => (selected.has(x.id) ? { ...x, isActive: false } : x)),
      );
      showToast(
        "success",
        `${selected.size} ZIP codes deactivated (not deleted)`,
      );
      setSelected(new Set());
    } finally {
  ...
  const handleBulkActivate = async () => {
    ...
    try {
      ...
      setRecords((r) =>
        r.map((x) => (selected.has(x.id) ? { ...x, isActive: true } : x)),
      );
  ...
  const handleBulkDelete = async () => {
    ...
    if (phrase !== "DELETE") {
      if (phrase !== null)
        showToast("error", "Type DELETE exactly to confirm. Action cancelled.");
      return;
    }
    setBulkWorking(true);
    try {
  ...
  const handleSeed = async () => {
    if (
      !confirm(
        "This will import all ZIP codes from the default Massachusetts list. Existing ZIPs will not be overwritten. Continue?",
      )
    )
      return;
    setSeeding(true);
    try {
      const res = await fetch("/api/admin/service-areas/seed", {
        method: "POST",
      });
      const d = await res.json();
      await load();
  ...
  return (
    <div className="max-w-6xl mx-auto pb-12" style={{ fontFamily: FN }}>
      ...
      {showAdd && (
        <AddZipModal onAdd={handleAdd} onClose={() => setShowAdd(false)} />
      )}
      {editRec && (
        <EditModal
          record={editRec}
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-slate-500 font-semibold text-sm ml-1">
            Manage Massachusetts ZIP codes — active ZIPs appear in customer
            booking form
          </p>
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          <button
            onClick={handleSeed}
            disabled={seeding}
            className="flex items-center gap-2 px-5 py-3 rounded-2xl text-sm font-black border-2 border-slate-200 text-slate-600 hover:border-[#FFA000] hover:text-[#000223] transition-all bg-white disabled:opacity-50"
          >
            {seeding ? (
              <Loader2 className="w-4 h-4 animate-spin" />
            ) : (
          ...
          <button
            onClick={exportCsv}
            className="flex items-center gap-2 px-5 py-3 rounded-2xl text-sm font-black border-2 border-slate-200 text-slate-600 hover:border-slate-400 bg-white transition-all"
          >
            <Download className="w-4 h-4" /> Export CSV
          </button>
          <button
            onClick={() => setShowAdd(true)}
            className="flex items-center gap-2 px-6 py-3 rounded-2xl text-sm font-black shadow-md hover:shadow-lg hover:-translate-y-0.5 transition-all"
            style={{ background: "#000223", color: "#FFA000" }}
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        {[
          ...
          <div
            key={s.label}
            className="bg-white rounded-[2rem] border border-slate-100 shadow-[0_8px_30px_rgba(0,0,0,0.015)] px-6 py-5 flex items-center gap-4"
          >
            <div
            ...
            <div className="text-sm font-bold text-[#000223]/70 leading-tight">
              {s.label}
            </div>
          </div>
        ))}
      ...
      <div className="bg-white rounded-[2rem] border border-slate-100 shadow-[0_8px_30px_rgba(0,0,0,0.015)] p-5 mb-5 flex flex-col md:flex-row gap-4 items-center">
        ...
        <button
          onClick={load}
          className="p-3.5 rounded-2xl border-2 border-slate-100 hover:border-slate-300 text-slate-400 hover:text-slate-600 transition-all shrink-0 bg-white"
        >
      ...
      {selected.size > 0 && (
        <div className="flex flex-wrap items-center gap-3 px-5 py-4 bg-amber-50/50 border border-amber-200/60 rounded-3xl mb-4 animate-in slide-in-from-top-2 duration-200">
          ...
          <button
            ...
            className="ml-auto text-amber-700 hover:text-amber-900 text-xs font-bold uppercase tracking-wider"
          >
            Clear
          </button>
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-100 bg-[#FAF6EF]">
                ...
                <th className="px-6 py-4 text-left text-xs font-black uppercase tracking-wider text-slate-400 hidden md:table-cell">
                  County
                </th>
                <th className="px-6 py-4 text-left text-xs font-black uppercase tracking-wider text-slate-400">
                  Status
                </th>
                <th className="px-6 py-4 text-left text-xs font-black uppercase tracking-wider text-slate-400 hidden lg:table-cell">
                  Notes
                </th>
              ...
              {loading ? (
                <tr>
                  <td colSpan={7} className="text-center py-16 bg-white">
                    <Loader2 className="w-6 h-6 animate-spin mx-auto text-[#FFA000]" />
                  </td>
                </tr>
              ) : filtered.length === 0 ? (
                <tr>
                  <td colSpan={7} className="text-center py-16 bg-white">
                    ...
                    <p className="text-xs text-slate-300 font-semibold mt-1">
                      Click "Seed MA Defaults" to import the default
                      Massachusetts list
                    </p>
                  </td>
                </tr>
                ...
                filtered.map((r) => (
                  <tr
                    key={r.id}
                    className={`hover:bg-amber-50/20 transition-colors ${selected.has(r.id) ? "bg-amber-50/30" : ""}`}
                  >
                    ...
                    <td className="px-6 py-4.5">
                      <span className="font-mono font-black text-sm text-[#000223]">
                        {r.zip}
                      </span>
                    </td>
                    <td className="px-6 py-4.5">
                      <span className="font-bold text-[#000223]/80">
                        {r.city}
                      </span>
                    </td>
                    ...
                    <td className="px-6 py-4.5">
                      ...
                      >
                        {r.isActive ? (
                          ...
                          <>
                            <ToggleLeft className="w-5 h-5 text-slate-300 group-hover:text-slate-400 transition-colors" />
                            <span className="text-xs font-black text-slate-400 bg-slate-100 border border-slate-200 px-2 py-0.5 rounded-lg">
                              Inactive
                            </span>
                          </>
                        )}
                    ...
                    <td className="px-6 py-4.5 hidden lg:table-cell">
                      <span className="text-xs text-slate-400 font-semibold truncate max-w-48 block">
                        {r.notes || "—"}
                      </span>
                    </td>
                    <td className="px-6 py-4.5 text-right">
                      <div className="flex items-center justify-end gap-1">
                        <button
                          ...
                          title="Edit"
                          className="p-2 rounded-lg hover:bg-amber-50 text-slate-400 hover:text-amber-600 transition-all hover:scale-105"
                        >
                          <Edit2 className="w-4 h-4" />
                        </button>
                        <button
                          onClick={() => handleDelete(r.id, r.zip)}
                          title="Permanent delete"
                          className="p-2 rounded-lg hover:bg-red-50 text-slate-300 hover:text-red-500 transition-all hover:scale-105"
                        >
      ...
      <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="flex items-start gap-3.5 px-6 py-5 bg-emerald-50/50 border border-emerald-100/60 rounded-3xl">
          ...
          <div>
            <p className="text-sm font-black text-emerald-850">
              Safe Way to Remove: Deactivate
            </p>
            <p className="text-xs font-semibold text-emerald-600 mt-1 leading-relaxed">
              Toggle or bulk-deactivate ZIPs to hide them from customers. The
              record stays in DB. You can reactivate at any time.
            </p>
          </div>
        </div>
        <div className="flex items-start gap-3.5 px-6 py-5 bg-blue-50/50 border border-blue-100/60 rounded-3xl">
          ...
          <div>
            ...
            <p className="text-xs font-semibold text-blue-600 mt-1 leading-relaxed">
              Default seed covers{" "}
              <strong>Greater Boston + surrounding MA cities</strong> (~120
              ZIPs). Add more manually or import a full MA list for statewide
              coverage.
            </p>
          </div>
        </div>
