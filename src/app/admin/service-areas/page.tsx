    ...
    <div className="max-w-6xl mx-auto pb-12" style={{ fontFamily: FN }}>
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-slate-500 font-semibold text-sm ml-13">Manage Massachusetts ZIP codes — active ZIPs appear in customer booking form</p>
        </div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        {[
          ...
          <div key={s.label} className="bg-white rounded-2xl border border-slate-200 shadow-sm px-5 py-4 flex items-center gap-4">
            <div className="w-12 h-12 rounded-xl flex items-center justify-center text-xl font-black shrink-0" style={{ background: s.bg, color: s.color }}>{s.value}</div>
            <div className="text-sm font-bold text-slate-500 leading-tight">{s.label}</div>
          </div>
        ))}
      </div>
      ...
      {/* Bulk actions bar */}
      {selected.size > 0 && (
        <div className="flex flex-wrap items-center gap-3 px-4 py-3 bg-amber-50 border border-amber-200 rounded-2xl mb-3">
          <span className="text-sm font-black text-amber-800">{selected.size} selected</span>

          {/* Primary: Deactivate (safe) */}
          <button onClick={handleBulkDeactivate} disabled={bulkWorking}
            className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-black border-2 border-amber-400 text-amber-800 hover:bg-amber-100 transition-colors disabled:opacity-50">
            {bulkWorking ? <Loader2 className="w-3 h-3 animate-spin" /> : <EyeOff className="w-3 h-3" />}
            Deactivate Selected
          </button>

          {/* Secondary: Activate */}
          <button onClick={handleBulkActivate} disabled={bulkWorking}
            className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-black border-2 border-emerald-400 text-emerald-700 hover:bg-emerald-50 transition-colors disabled:opacity-50">
            {bulkWorking ? <Loader2 className="w-3 h-3 animate-spin" /> : <Eye className="w-3 h-3" />}
            Activate Selected
          </button>

          {/* Danger: Delete (requires typed confirm) */}
          <button onClick={handleBulkDelete} disabled={bulkWorking}
            className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-black border-2 border-red-300 text-red-500 hover:bg-red-50 transition-colors disabled:opacity-50">
                ...
                <tr key={r.id} className={`hover:bg-amber-50/30 transition-colors ${selected.has(r.id) ? "bg-amber-50/50" : ""}`}>
                  ...
                  <td className="px-4 py-3 text-right">
                    <div className="flex items-center justify-end gap-1">
                      <button onClick={() => setEditRec(r)} title="Edit" className="p-1.5 rounded-lg hover:bg-amber-50 text-slate-400 hover:text-amber-600 transition-colors">
                        <Edit2 className="w-3.5 h-3.5" />
                      </button>
                      <button onClick={() => handleDelete(r.id, r.zip)} title="Permanent delete" className="p-1.5 rounded-lg hover:bg-red-50 text-slate-300 hover:text-red-500 transition-colors">
      ...
      <div className="mt-4 grid md:grid-cols-2 gap-3">
        <div className="flex items-start gap-3 px-5 py-4 bg-emerald-50 border border-emerald-100 rounded-2xl">
          <Eye className="w-5 h-5 text-emerald-500 shrink-0 mt-0.5" />
          <div>
            <p className="text-sm font-black text-emerald-800">Safe Way to Remove: Deactivate</p>
        ...
        <div className="flex items-start gap-3 px-5 py-4 bg-blue-50 border border-blue-100 rounded-2xl">
          ...
          <div>
            <p className="text-sm font-black text-blue-800">Coverage Note</p>
