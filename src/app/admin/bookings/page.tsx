...
export default function AdminBookingsPage() {
  ...
  const counts = {
    ALL: bookings.length,
    PENDING_REVIEW: bookings.filter((b) => b.status === "PENDING_REVIEW")
      .length,
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 font-semibold mt-1 text-sm">
            {counts.PENDING_REVIEW} pending review · {counts.PENDING_PAYMENT}{" "}
            pending payment · {counts.CONFIRMED} confirmed
          </p>
        </div>
          ...
          <div className="py-24 text-center">
            ...
            <p className="text-sm font-semibold text-slate-400 mt-1">
              {search
                ...
                : "Bookings will appear here when customers reserve"}
            </p>
          </div>
        ) : (
            ...
            <table className="w-full text-left border-collapse min-w-[850px]">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-200">
                  <th className="px-5 py-3.5 text-xs font-black uppercase tracking-wider text-slate-400">
                    Booking
                  </th>
                  <th className="px-5 py-3.5 text-xs font-black uppercase tracking-wider text-slate-400">
                    Customer
                  ...
                  <th className="px-5 py-3.5 text-xs font-black uppercase tracking-wider text-slate-400 text-right">
                    Actions
                  </th>
                </tr>
              </thead>
                    ...
                    >
                      <td className="px-5 py-4">
                        ...
                        <div className="text-xs text-slate-400 font-semibold mt-0.5">
                          {b.guests} guests · {b.durationMins} min
                        </div>
                      </td>
                      <td className="px-5 py-4">
                      ...
                      <td className="px-5 py-4">
                        <div className="font-black text-sm text-[#000223]">
                          ...
                          {b.quote?.totalAmount?.toFixed(0) ??
                            b.package?.basePrice?.toFixed(0) ??
                            "—"}
                        </div>
                      </td>
                      <td className="px-5 py-4">
                        <span
                          className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-black ${sc.bg} ${sc.text}`}
                        >
