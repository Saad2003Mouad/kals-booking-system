...
export default function AdminBookingDetailPage({
  ...
  const loadBooking = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setCustomPrice(
          json.data.totalAmount > 0 ? String(json.data.totalAmount) : "",
        );
  ...
  if (!booking)
    return <div className="p-24 text-center">Booking not found</div>;
  ...
  const packageName =
    breakdown.packageName ?? (booking.package?.name || "Custom Package");
  const packagePrice =
    breakdown.packagePrice ?? booking.quote?.basePrice ?? 250;
  const includedGuests =
    breakdown.includedGuests ?? booking.package?.servings ?? 50;
  const includedServiceMins =
    breakdown.includedServiceMins ??
    (booking.package as any)?.durationMins ??
    booking.package?.includedMinutes ??
    booking.durationMins;
  const extraGuestsFee =
    breakdown.additionalGuestsFee ?? booking.quote?.extraPieceFee ?? 0;
  const extraServiceFee =
    breakdown.additionalServiceFee ?? booking.extraServiceFee ?? 0;
  ...
  const additionalLocationServiceFee =
    breakdown.additionalLocationServiceFee ?? additionalStopsFee;
  ...
  return (
    <div className="max-w-4xl mx-auto pb-12 animate-in fade-in zoom-in duration-300">
      <Link
        href="/admin/bookings"
        className="inline-flex items-center gap-2 text-sm text-[#000223]/70 hover:text-[#000223] mb-6 font-bold bg-white border border-slate-100 shadow-sm px-4 py-2 rounded-xl transition-all hover:-translate-y-0.5"
      >
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
        <div>
          <h2 className="text-3xl font-black text-[#000223] flex items-center gap-3">
            ...
            <span
              className={`px-3 py-1 text-[11px] rounded-full font-black uppercase tracking-widest ${
        ...
        {booking.status === "PENDING_REVIEW" &&
          (() => {
            const isCustomPackage =
              ...
              booking.package?.serviceType === "CUSTOM";
              ...
              : booking.quote?.snapshotJson
                ? (() => {
                    try {
                      ...
                      if (
                        snap.aiFlags?.includes(
                          "LONG_DISTANCE_LOW_PACKAGE_VALUE",
                        )
                      ) {
              ...
              <div className="flex flex-col gap-4 p-5 bg-amber-50 rounded-2xl border border-amber-200 w-full md:w-auto shadow-sm">
                ...
                {isCustomPackage && (
                  <div className="space-y-4 border-t border-amber-200 pt-4">
                    ...
                    <div>
                      <label className="block text-xs font-black text-amber-900 uppercase tracking-wider mb-1">
                        Customer-facing Notes / Pricing Notes
                      </label>
                      <textarea
                        value={customerNotes}
                ...
                <div className="flex gap-3 pt-2">
                  <button
                    ...
                    className="btn-secondary py-2.5 px-6 text-sm text-red-650 border-red-200 hover:bg-red-50 disabled:opacity-50 flex items-center gap-2"
                  >
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <div className="card-premium p-6">
          <h3 className="text-lg font-black mb-4 flex items-center gap-2 text-[#000223]">
            <User className="w-5 h-5 text-[#FFA000]" /> Customer Details
          </h3>
        ...
        <div className="card-premium p-6">
          <h3 className="text-lg font-black mb-4 flex items-center gap-2 text-[#000223]">
            <CalendarClock className="w-5 h-5 text-blue-500" /> Event Details
          </h3>
          <div className="space-y-4 text-sm font-semibold">
            <div className="flex justify-between items-center">
            ...
            <div className="flex justify-between items-center">
              ...
              <span className="text-slate-800">{booking.startTime}</span>
            </div>
            <div className="flex justify-between items-center">
              ...
              <span className="text-slate-800">
                ...
                {breakdown.additionalServiceMins
                  ? `(+${breakdown.additionalServiceMins} mins)`
                  : ""}
              </span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-slate-400">Guests:</span>{" "}
              <span className="text-slate-800">{booking.guests}</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-slate-400">Type:</span>{" "}
              <span className="text-slate-800">{booking.eventType}</span>
            </div>
          </div>
        </div>
        ...
        <div className="card-premium p-6 md:col-span-2">
          <h3 className="text-lg font-black mb-4 flex items-center gap-2 text-[#000223]">
            <MapPin className="w-5 h-5 text-rose-500" /> Route & Stops
          </h3>
          ...
          <div className="space-y-4">
            <div className="flex flex-col border-l-2 border-slate-200 pl-4 py-1 relative">
              ...
              <span className="text-xs font-black uppercase text-emerald-600 mb-1">
                Primary Location
              </span>
              <span className="font-bold text-[#000223] text-sm">
                {booking.address}
              </span>
              <span className="font-semibold text-slate-500 text-xs">
                {booking.city}, {booking.zip}
              </span>
            </div>
            ...
            {booking.stops?.map((stop: any, idx: number) => (
              <div
                key={stop.id}
                className="flex flex-col border-l-2 border-slate-200 pl-4 py-1 relative"
              >
                <div className="absolute w-3 h-3 bg-blue-500 rounded-full -left-[7px] top-2 border-2 border-white"></div>
                <span className="text-xs font-black uppercase text-blue-600 mb-1">
                  Additional Stop {idx + 1}
                </span>
                <span className="font-bold text-[#000223] text-sm">
                  {stop.street}
                </span>
                ...
                {stop.notes && (
                  <span className="font-semibold text-slate-400 text-xs mt-1.5 italic">
                    📝 {stop.notes}
                  </span>
                )}
              </div>
        ...
        <div className="card-premium p-6">
          ...
          {(() => {
            ...
            return (
              <div className="mb-4 p-3 bg-slate-50 border border-slate-100 rounded-xl flex justify-between items-center font-bold text-xs">
                ...
                <span
                  className={`px-2.5 py-0.5 rounded-full uppercase tracking-wider text-[10px] ${
                    isCustomPackage
          ...
          {(() => {
              ...
              return (
                <div className="py-4 border-b border-slate-100 mb-4 text-sm font-semibold text-slate-500 space-y-2">
                  <p className="text-xs text-amber-700 bg-amber-50 border border-amber-250 p-2.5 rounded-lg leading-relaxed">
                    ⚠️ <strong>Pending Quote Review:</strong> This booking has
                    no fixed price yet. Please review details, compute travel
                    and weekend fees, and enter a final custom price.
                  </p>
                </div>
              );
            }
            return (
              <div className="space-y-3 text-sm font-semibold pb-4 border-b border-slate-100 mb-4 text-slate-800">
                ...
                <div className="flex justify-between items-center">
                  <span className="text-slate-400">Vehicles Required:</span>{" "}
                  <span>{vehiclesRequired}</span>
                </div>
                ...
                {extraGuestsFee > 0 && (
                  <div className="flex justify-between items-center">
                    <span className="text-slate-400">Extra Guests Fee:</span>{" "}
                ...
                {additionalLocationServiceFee > 0 && (
                  <div className="flex justify-between items-center">
                    <span className="text-slate-400">
                      Additional Location Service Fee:
                    </span>{" "}
                ...
                {weekendFee > 0 && (
                  <div className="flex justify-between items-center">
                    <span className="text-slate-400">Weekend Event Fee:</span>{" "}
          ...
          {(() => {
            ...
            const day = new Date(
              booking.eventDate.toString().split("T")[0] + "T12:00:00",
            ).getDay();
            ...
            if (isWeekendDay) {
              return (
                <div className="mt-4 p-3 rounded-xl text-xs font-bold text-blue-900 bg-blue-50 border border-blue-200 leading-relaxed">
                  📅 <strong>Weekend Event:</strong> Saturday/Sunday bookings
                  include a $25 weekend event fee. Please verify this is
                  factored into the custom total.
                </div>
              );
            }
            return null;
        ...
        <div className="card-premium p-6 flex flex-col justify-between">
          <div>
            <h3 className="text-lg font-black mb-4 flex items-center gap-2 text-[#000223]">
              <Truck className="w-5 h-5 text-[#FFA000]" /> Dispatch Assignment
            </h3>
      ...
      {booking.status === "PENDING_REVIEW" && (
        <div className="mt-6 card-premium p-8 border-amber-200 bg-amber-50/20">
          <label className="block text-sm font-black text-amber-900 mb-2 uppercase tracking-wider">
          ...
          <textarea
            ...
            rows={4}
            placeholder="E.g. Travel fee adjusted due to traffic conditions..."
          />
        </div>
      )}
    </div>
