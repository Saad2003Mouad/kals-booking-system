        ...
        <div className="px-6 py-6 border-b border-white/5 flex items-center gap-4">
          <div className="w-12 h-12 rounded-full bg-gradient-to-tr from-[#FFA000] to-[#FFD000] p-0.5 shadow-[0_0_15px_rgba(255,160,0,0.3)] flex-shrink-0">
            <div className="w-full h-full rounded-full bg-[#020617] flex items-center justify-center border-2 border-transparent">
              <span className="text-[#FFA000] font-bold text-lg">
                {userInitials}
              </span>
            </div>
          </div>
          <div className="min-w-0 flex-1">
            ...
            <div className="text-emerald-400 text-xs font-semibold uppercase mt-0.5 tracking-wider">
              {userRole}
            </div>
          </div>
        </div>
          ...
          {NAV_GROUPS.map((group, i) => {
            ...
            return (
              <div key={i} className="space-y-1">
                <div className="text-[10px] font-black text-white/40 uppercase tracking-widest mb-3 px-4">
                  {group.title}
                </div>
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <Icon
                          className={`w-4 h-4 ${active ? "text-[#FFA000]" : "text-white/40 group-hover:text-white/80"}`}
                        />
                        {label}
                      </div>
                      {active && (
                        <ChevronRight className="w-4 h-4 text-[#FFA000]/50" />
                      )}
                    </Link>
                  );
                })}
        ...
        <header className="h-20 flex-shrink-0 flex items-center justify-between px-4 sm:px-8 border-b border-slate-200 bg-white/80 backdrop-blur-xl sticky top-0 z-30">
          ...
          <div className="flex items-center gap-4 sm:gap-6">
            ...
            <div className="hidden lg:flex items-center relative group">
              ...
              />
              <div className="absolute right-3 px-1.5 py-0.5 rounded text-[10px] font-bold text-slate-400 border border-slate-200">
                ↵
              </div>
            </div>
              ...
              {showNotifications && (
                <div className="absolute right-0 top-full mt-2 w-96 bg-white rounded-2xl shadow-2xl border border-slate-200 overflow-hidden z-50 animate-in slide-in-from-top-2 duration-200">
                  <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
                    <span className="font-black text-[#000223] text-sm">
                      Notifications
                    </span>
                    {unreadCount > 0 && (
                      <button
                        onClick={markAllRead}
                        className="text-xs font-bold text-[#FFA000] hover:underline"
                      >
                        Mark all read
                      </button>
                    )}
                  ...
                  <div className="max-h-80 overflow-y-auto divide-y divide-slate-50">
                    {notifications.length === 0 ? (
                      ...
                      notifications.map((n) => (
                        <div
                          key={n.id}
                          className={`px-5 py-3.5 flex gap-3 hover:bg-slate-50 transition-colors ${!n.read ? "bg-amber-50/40" : ""}`}
                        >
                          <div
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="font-bold text-xs text-[#000223]">
                              {n.title}
                            </div>
              ...
              {showUserMenu && (
                <div className="absolute right-0 top-full mt-2 w-56 bg-white rounded-2xl shadow-2xl border border-slate-200 overflow-hidden z-50 animate-in slide-in-from-top-2 duration-200">
                  <div className="px-4 py-3 border-b border-slate-100 bg-slate-50">
                    <div className="font-black text-sm text-[#000223] truncate">
                      {userName}
                    </div>
