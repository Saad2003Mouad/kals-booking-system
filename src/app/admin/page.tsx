...
function StatCard({
  ...
  return (
    <Link
      href={href}
      className="bg-white rounded-2xl p-6 shadow-sm border border-slate-200/60 hover:shadow-md hover:border-[#FFA000]/30 transition-all group flex flex-col justify-between h-full relative overflow-hidden"
    >
      <div
        className={`absolute -right-6 -top-6 w-24 h-24 rounded-full opacity-10 ${bgClass} blur-xl pointer-events-none group-hover:scale-150 transition-transform duration-500`}
      />
      <div className="flex justify-between items-start mb-4 relative z-10">
        <div
          className={`w-12 h-12 rounded-xl flex items-center justify-center ${bgClass} bg-opacity-10 text-opacity-100 shadow-sm border border-white`}
        >
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
        {[
          ...
          <div
            key={s.label}
            className="bg-white rounded-2xl border border-slate-200/60 shadow-sm p-6 text-center hover:shadow-md transition-all"
          >
              ...
              <div className="p-6">
                <div className="flex justify-between items-start mb-5">
                  <div>
                    <div className="font-black text-xl text-slate-800">
                      #{selected.booking.bookingNumber}
                    </div>
                ...
                <div className="space-y-3.5 mb-6 rounded-2xl p-4 bg-slate-50 border border-slate-100">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-slate-400 font-semibold">
                        {selected.booking.durationMins} min event
                      </div>
                    </div>
                  </div>
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      <div className="font-bold text-sm text-slate-700">
                        {selected.booking.address}
                      </div>
                      <div className="text-xs text-slate-400 font-semibold">
                        {selected.booking.city}, MA {selected.booking.zip}
                      </div>
                      {selected.booking.stops &&
                        selected.booking.stops.length > 0 && (
                          <div className="mt-3 space-y-3">
                            {selected.booking.stops.map(
                              (stop: any, idx: number) => (
                                <div
                                  key={stop.id}
                                  className="relative pl-3 border-l-2 border-blue-200"
                                >
                                  <div className="absolute w-2 h-2 bg-blue-500 rounded-full -left-[5px] top-1.5"></div>
                                  <div className="font-bold text-xs text-blue-600 uppercase mb-0.5">
                                    Stop {idx + 1}
                                  </div>
                                  <div className="font-bold text-sm text-slate-700">
                                    {stop.street}
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <a
                      href={`tel:${selected.booking.customer.phone}`}
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="font-semibold text-sm text-slate-700">
                      {selected.booking.guests} guests ·{" "}
                      {selected.booking.package?.name ?? ""}
                    </div>
                  </div>
                ...
                <div className="font-black text-xs uppercase tracking-widest text-slate-400 mb-3">
                  Update Status
                </div>
...
export default function AdminDashboard() {
      ...
      <div className="space-y-6 animate-pulse">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-5">
          {[...Array(6)].map((_, i) => (
            <div
              key={i}
              className="h-36 bg-white rounded-2xl border border-slate-100 shadow-sm"
            ></div>
          ))}
        </div>
  ...
  if (!data) return null;
  const { stats, todayBookings, pendingBookings, vehicles, revenueChart } =
    data;
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-2">
        ...
        <Link
          href="/booking"
          target="_blank"
          className="inline-flex items-center justify-center gap-2 px-6 py-2.5 rounded-xl bg-[#020617] text-white hover:bg-[#FFA000] hover:text-[#020617] font-bold text-sm shadow-md transition-all"
        >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-5">
        <StatCard
          ...
          value={stats.completedMonth + stats.pending}
          icon={CalendarDays}
          href="/admin/bookings"
          bgClass="bg-blue-500"
        ...
        <StatCard
          label="Active Fleet"
          value={vehicles.filter((v) => v.status === "ON_JOB").length}
          icon={Truck}
          href="/admin/vehicles"
          bgClass="bg-[#FFA000]"
      ...
      <div className="grid xl:grid-cols-3 gap-6">
        ...
        <div
          className={`${userRole === "SUPPORT" ? "xl:col-span-3" : "xl:col-span-2"} space-y-6`}
        >
          {/* Revenue Chart */}
          {userRole !== "SUPPORT" && (
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200/60 overflow-hidden flex flex-col">
            <div className="px-6 py-5 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
              <h2 className="font-bold text-lg text-slate-800 flex items-center gap-2">
                <CalendarDays className="w-5 h-5 text-blue-500" /> Today's
                Operations
              </h2>
              <Link
                href="/admin/bookings"
                    ...
                    {todayBookings.map((b) => (
                      ...
                      >
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-right">
                          <span
                            className={`inline-flex px-3 py-1 rounded-full text-[11px] font-black tracking-wide ${STATUS_COLOR[b.status] ?? "bg-slate-100 text-slate-600"}`}
                          >
                            {b.status.replace("_", " ")}
                          </span>
                        </td>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200/60 overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center">
              <h2 className="font-bold text-base text-slate-800 flex items-center gap-2">
                <AlertCircle className="w-5 h-5 text-amber-500" /> Action
                Required
            ...
            {pendingBookings.length === 0 ? (
              ...
              <div className="divide-y divide-slate-50 max-h-[350px] overflow-y-auto">
                {pendingBookings.slice(0, 5).map((b) => (
                  ...
                  >
                    ...
                    <Link
                      ...
                      className="flex-shrink-0 w-8 h-8 rounded-full bg-white border border-slate-200 flex items-center justify-center text-slate-400 hover:text-[#FFA000] hover:border-[#FFA000] hover:bg-[#FFA000]/10 transition-all"
                    >
                ...
                {pendingBookings.length > 5 && (
                  <Link
                    href="/admin/bookings?status=PENDING"
                    className="block p-4 text-center text-sm font-bold text-amber-600 hover:bg-amber-50 transition-colors"
                  >
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200/60 overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
              <h3 className="font-bold text-base text-slate-800 flex items-center gap-2">
                <Truck className="w-4 h-4 text-slate-400" /> Fleet Status
              </h3>
              <Link
                href="/admin/vehicles"
            ...
            <div className="p-2">
              {vehicles.map((v) => {
                const c = VEHICLE_COLOR[v.status] ?? {
                ...
                return (
                  ...
                  >
                    ...
                    <span
                      className={`text-[10px] font-black uppercase tracking-wider px-2.5 py-1 rounded-full ${c.bg} ${c.text}`}
                    >
                      {v.status.replace("_", " ")}
                    </span>
                  </div>
