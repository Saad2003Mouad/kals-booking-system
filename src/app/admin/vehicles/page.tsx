...
const TRUCK_IMG =
  "https://cdn.prod.website-files.com/67dc601bc29781a5af1632a2/68370bab3a2a59b9eecd7910_5429ba7e106f479fe18b0f9ad0cf5de3_boston-legend-ice-cream-truck-white-header-bg.avif";
const VAN_IMG =
  "https://cdn.prod.website-files.com/67dc601bc29781a5af1632a2/67e3936366827af4bed1d0d0_logo-boston-legend-ice-cream-truck.avif"; // Logo or fallback sprinter icon
...
export default function VehiclesPage() {
  ...
  const remove = async (id: string) => {
    if (
      !confirm(
        "Are you sure you want to remove this vehicle from the active fleet?",
      )
    )
      return;
    try {
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 font-semibold mt-1 text-sm">
            Monitor live availability, driver assignments, and maintenance logs
          </p>
        </div>
        <button
          onClick={openAdd}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
        {[
          ...
          >
            ...
            <div className="text-xs text-slate-450 font-black uppercase tracking-widest">
              {s.label}
            </div>
          </div>
        ))}
      ...
      {loading ? (
        ...
        <div className="py-24 text-center bg-white rounded-2xl border border-slate-200 shadow-sm">
          ...
          <p className="text-sm font-semibold text-slate-450 mt-1">
            Try adjusting search parameters or add a new vehicle.
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {filtered.map((v) => {
            ...
            return (
              <div
                key={v.id}
                className="bg-white rounded-2xl border border-slate-200/80 shadow-sm hover:shadow-md transition-all overflow-hidden flex flex-col group hover:-translate-y-0.5"
              >
                ...
                <div className="relative h-36 bg-slate-50 border-b border-slate-100 flex items-center justify-center p-6 overflow-hidden">
                  ...
                  {/* Status Tag Overlay */}
                  <div
                    className={`absolute top-3 right-3 flex items-center gap-1.5 px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-wider backdrop-blur-md bg-white/95 border ${SInfo.border} shadow-sm ${SInfo.text}`}
                  >
                    <span
                      className={`w-1.5 h-1.5 rounded-full ${SInfo.dot} animate-pulse`}
                    />
                    {SInfo.label}
                  </div>
                </div>
                ...
                <div className="p-5 flex-grow flex flex-col justify-between">
                  <div>
                    <div className="flex justify-between items-start mb-2">
                      <div>
                        <div className="text-[10px] font-black text-[#FFA000] uppercase tracking-widest">
                          {v.code}
                        </div>
                        <h3 className="text-[15px] font-black tracking-tight text-[#000223] mt-0.5 truncate">
                          {v.name}
                        </h3>
                      </div>
                    ...
                    <div className="mt-4 space-y-2 text-xs font-bold text-slate-500">
                      <div className="flex items-center gap-2">
                        ...
                        {v.driver ? (
                          <span>
                            Assigned:{" "}
                            <span className="text-[#000223] font-black">
                              {v.driver}
                            </span>
                          </span>
                        ) : (
