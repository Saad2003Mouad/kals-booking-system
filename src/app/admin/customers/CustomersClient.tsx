...
export default function CustomersClient({
  ...
  initialCustomers: CustomerWithBookings[];
}) {
  const [search, setSearch] = useState("");
  const [selectedCustomer, setSelectedCustomer] =
    useState<CustomerWithBookings | null>(null);
  ...
  const totalSpent = initialCustomers.reduce(
    (sum, c) =>
      sum +
      c.bookings.reduce(
        (s, b) => s + (b.quote?.totalAmount ?? b.totalAmount ?? 0),
        0,
      ),
    0,
  );

  const repeatCustomersCount = initialCustomers.filter(
    ...
    >
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 font-semibold mt-1 text-sm">
            Manage {initialCustomers.length} premium client profiles and history
          </p>
        </div>
        <a
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
        <div className="bg-gradient-to-br from-[#000223] to-[#0a0f4a] rounded-2xl border border-white/5 shadow-xl p-6 relative overflow-hidden group">
          ...
          <div className="text-3xl sm:text-4xl font-black text-[#FFA000] mb-1 tracking-tight">
            {initialCustomers.length}
          </div>
          <div className="text-xs text-slate-350 font-black uppercase tracking-widest">
            Total Clients
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200/80 shadow-md p-6 relative overflow-hidden group">
          ...
          <div className="text-3xl sm:text-4xl font-black text-emerald-600 mb-1 tracking-tight">
            $
        ...
        <div className="bg-white rounded-2xl border border-slate-200/80 shadow-md p-6 relative overflow-hidden group">
          ...
          <div className="text-3xl sm:text-4xl font-black text-[#000223] mb-1 tracking-tight">
            {initialCustomers.length > 0
              ? (
                  (repeatCustomersCount / initialCustomers.length) *
                  100
                ).toFixed(0)
              : 0}
            %
          ...
          <div className="text-xs text-slate-400 font-black uppercase tracking-widest">
            Repeat Rate ({repeatCustomersCount} clients)
          </div>
        </div>
      </div>
      ...
      {filtered.length === 0 ? (
        <div className="py-24 text-center bg-white rounded-2xl border border-slate-200/80 shadow-sm">
          ...
          <h3 className="text-lg font-black text-[#000223]">
            No customers match your query
          </h3>
          <p className="text-sm font-semibold text-slate-500 mt-1 max-w-xs mx-auto">
            Double check spelling or add a new customer from bookings
          </p>
        </div>
      ) : (
                  ...
                  <div className="mt-5 grid grid-cols-2 gap-4">
                    <div className="bg-[#FAF8F0]/40 rounded-xl p-3 border border-slate-100">
                      <div className="text-[9px] font-black uppercase text-slate-400 tracking-wider mb-1 flex items-center gap-1">
                        <CalendarDays className="w-3 h-3 text-[#FFA000]" />{" "}
                        Bookings
                      </div>
                      <div className="font-black text-[#000223] text-sm">
                        {c.bookings.length} Events
                      </div>
                    </div>
                    <div className="bg-[#FAF8F0]/40 rounded-xl p-3 border border-slate-100">
                      <div className="text-[9px] font-black uppercase text-slate-400 tracking-wider mb-1 flex items-center gap-1">
      ...
      {selectedCustomer && (
        <div
          className="fixed inset-0 z-50 overflow-hidden print:hidden"
                  ...
                  <div className="flex-1 p-6 space-y-8">
                        ...
                        <div className="flex justify-between items-center">
                          ...
                          {selectedCustomer.email ? (
                            <a
                              ...
                              className="text-[#000223] hover:text-[#FFA000] font-black underline truncate max-w-[200px]"
                            >
                              {selectedCustomer.email}
                            </a>
                          ) : (
                        ...
                        <div className="flex justify-between items-center">
                          ...
                          {selectedCustomer.phone ? (
                            <a
                              ...
                              className="text-[#000223] hover:text-[#FFA000] font-black"
                            >
                              {selectedCustomer.phone}
                            </a>
                          ) : (
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="bg-[#FAF8F0]/40 rounded-xl p-4 border border-slate-200/85">
                        <div className="text-[10px] font-black uppercase text-slate-450 tracking-wider mb-1">
                          Total Bookings
                        </div>
                        <div className="text-2xl font-black text-[#000223]">
                        ...
                        <div className="text-2xl font-black text-emerald-600">
                          ...
                          {selectedCustomer.bookings
                            .reduce(
                              (s, b) =>
                                s +
                                (b.quote?.totalAmount ?? b.totalAmount ?? 0),
                              0,
                            )
                            .toFixed(0)}
                        </div>
                    ...
                    <div className="space-y-4">
                      <h3 className="text-xs font-black uppercase tracking-widest text-slate-400 flex items-center gap-1.5">
                        <Calendar className="w-4 h-4 text-[#FFA000]" /> Booking
                        History
                      </h3>

                      {selectedCustomer.bookings.length === 0 ? (
                        <p className="text-xs text-slate-400 italic font-semibold pl-2">
                        ...
                        <div className="space-y-4">
                          {selectedCustomer.bookings.map((b) => {
                            ...
                            return (
                              <div
                                key={b.id}
                                className="relative pl-6 pb-2 border-l border-slate-200/80 last:border-0 last:pb-0"
                              >
                                ...
                                <div className="bg-[#FAF8F0]/30 rounded-xl border border-slate-200/80 p-4 hover:border-slate-300 transition-all">
                                  <div className="flex justify-between items-start mb-1">
                                    <span className="font-mono text-xs font-black text-[#000223]">
                                      #{b.bookingNumber}
                                    </span>
                                  ...
                                  <div className="flex justify-between items-center text-xs text-slate-500 font-semibold">
                                    ...
                                    <span
                                      className={`px-2 py-0.5 rounded text-[9px] font-black uppercase tracking-wide ${
                                        b.status === "CONFIRMED"
