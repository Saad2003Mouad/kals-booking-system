...
const ROLE_PERMISSIONS_DISPLAY: Record<string, string[]> = {
  ...
  DRIVER: ["View Assigned Jobs Only", "Update Assigned Job Status"],
  SUPPORT: [
    "Limited Dashboard Stats (No Revenue)",
    "View Bookings",
...
export default function AdminStaffPage() {
  ...
  useEffect(() => {
    if (loggedInRole === "OWNER") {
      ...
      setLoading(false);
      setError(
        "Access Denied: Only users with the OWNER role are authorized to manage staff and permissions.",
      );
    }
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const body = editingStaff
        ? { role: formData.role, permissions: rolePermissions }
      ...
      if (res.ok && json && json.success) {
        ...
        alert(
          json?.error ||
            "Failed to save user (Only OWNER role can perform this action)",
        );
      }
    } catch (err) {
      alert("Network error");
                  ...
                  {staff.map((s) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-6 py-4">
                          <span
                            className={`px-2.5 py-1 rounded-md text-[11px] font-black tracking-wide ${isTargetOwner ? "bg-purple-100 text-purple-700 border border-purple-200" : "bg-blue-50 text-blue-600 border border-blue-100"}`}
                          >
                            {s.role}
                          </span>
                        </td>
          ...
          <div className="space-y-4">
            <div className="bg-slate-50 border border-slate-200 rounded-2xl p-5">
              <h2 className="font-bold text-base text-slate-800 flex items-center gap-2 mb-3">
                <Info className="w-5 h-5 text-blue-500" /> Predefined Access
                Profiles
              </h2>
              <p className="text-xs font-semibold text-slate-500 leading-relaxed">
                Roles are managed by predefined permission profiles. Granular
                permissions are assigned automatically based on the selected
                System Role and enforced server-side.
              </p>
            </div>
                  ...
                  <div key={role} className="pt-3 first:pt-0">
                    ...
                    <div className="flex flex-wrap gap-1">
                      {ROLE_PERMISSIONS_DISPLAY[role]?.map((perm) => (
                        <span
                          key={perm}
                          className="text-[9px] font-bold text-slate-500 bg-slate-50 border border-slate-100 px-1.5 py-0.5 rounded"
                        >
                          {perm}
                        </span>
                      ))}
          ...
          <div className="bg-white rounded-3xl shadow-xl w-full max-w-lg overflow-hidden flex flex-col max-h-[90vh]">
            <div className="px-6 py-5 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
              <h2 className="font-bold text-lg text-slate-800">
                {editingStaff ? "Edit Staff Roles" : "Add Staff Member"}
              </h2>
              <button
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="input-premium py-2.5"
                    placeholder="john@bostonlegend.com"
                  />
                </div>
                {!editingStaff && (
                  <div>
                ...
                <div>
                  ...
                  <select
                    ...
                    className="input-premium py-2.5 font-bold text-slate-700"
                  >
                    {ROLES.map((r) => {
                      // Prevent non-owners from selecting OWNER, though page is OWNER only anyway
                      if (r === "OWNER" && loggedInRole !== "OWNER")
                        return null;
                      return (
