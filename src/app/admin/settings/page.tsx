...
const SETTING_GROUPS = [
    ...
    settings: [
      {
        key: "FREE_MILES",
        label: "Free Miles",
        helper: "Miles included free before travel fee starts",
        unit: "mi",
        default: "10",
      },
      {
        key: "RATE_PER_MILE",
        label: "Rate per Mile",
        helper: "Travel fee charged per mile beyond free miles ($)",
        unit: "$/mi",
        default: "2.25",
      },
      {
        key: "DISTANCE_REVIEW_THRESHOLD_MILES",
        label: "Distance Review Threshold",
        helper:
          "Miles beyond which bookings go to Pending Review (not rejected)",
        unit: "mi",
        default: "30",
      },
      {
        key: "MAX_DISTANCE_MILES",
        label: "Typical Max Range (soft flag)",
        helper:
          "Info flag only — adds EXCEEDS_TYPICAL_RANGE label. NEVER causes rejection",
        unit: "mi",
        default: "45",
      },
    ],
  },
  {
    ...
    settings: [
      {
        key: "AUTO_CONFIRM_THRESHOLD",
        label: "Auto-Confirm Threshold",
        helper:
          "Minimum total ($) for automatic confirmation without admin review",
        unit: "$",
        default: "500",
      },
    ],
  },
  {
    ...
    settings: [
      {
        key: "BUSINESS_START_WEEKDAY",
        label: "Weekday Open",
        helper: "Opening hour Mon–Fri, 24h (8 = 8:00 AM)",
        unit: "h",
        default: "8",
      },
      {
        key: "BUSINESS_END_WEEKDAY",
        label: "Weekday Close",
        helper: "Closing hour Mon–Fri, 24h (22 = 10:00 PM)",
        unit: "h",
        default: "22",
      },
      {
        key: "BUSINESS_START_WEEKEND",
        label: "Weekend Open",
        helper: "Opening hour Sat–Sun, 24h (9 = 9:00 AM)",
        unit: "h",
        default: "9",
      },
      {
        key: "BUSINESS_END_WEEKEND",
        label: "Weekend Close",
        helper: "Closing hour Sat–Sun, 24h (22 = 10:00 PM)",
        unit: "h",
        default: "22",
      },
    ],
    ...
    settings: [
      {
        ...
        helper: "Email address to receive chat escalations and alerts",
        unit: "",
        default: "ops@bostonlegend.com",
      },
    ],
  },
];
...
export default function SettingsPage() {
  ...
  const handleInitialSync = async () => {
    ...
    try {
      ...
      setToast({
        type: "success",
        msg: json.message || "Initial sync completed successfully.",
      });
    } catch (e: any) {
      setToast({
        type: "error",
        msg: e.message || "Failed to run initial sync.",
      });
    } finally {
      setSyncing(false);
  ...
  return (
    ...
    >
      ...
      {toast && (
        <div
          className={`fixed top-6 right-6 z-50 flex items-center gap-3 px-5 py-3.5 rounded-2xl shadow-xl text-sm font-bold transition-all ${toast.type === "success" ? "bg-emerald-50 border border-emerald-200 text-emerald-800" : "bg-red-50 border border-red-200 text-red-800"}`}
        >
          {toast.type === "success" ? (
      ...
      <div className="flex items-end justify-between mb-8">
        <div>
          ...
          <p className="text-slate-500 font-semibold mt-1 text-sm">
            Configure pricing, distance rules, and business hours. Changes take
            effect immediately.
          </p>
        </div>
        <div className="flex gap-2">
          {isDirty && (
            <button
              onClick={handleReset}
              className="px-4 py-2.5 rounded-xl text-sm font-black border-2 border-slate-200 text-slate-500 hover:border-slate-400 transition-all flex items-center gap-2"
            >
      ...
      <div className="space-y-6">
                    ...
                    >
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          {changed && (
                            <span className="text-[10px] font-black text-amber-600 bg-amber-50 border border-amber-200 px-1.5 py-0.5 rounded-lg">
                              unsaved
                            </span>
                          )}
                        </div>
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        <input
                          ...
                          style={{
                            ...
                            color: "#000223",
                            fontFamily: s.key.includes("EMAIL")
                              ? "inherit"
                              : "monospace",
                          }}
        ...
        <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
          <div className="px-6 py-4 border-b border-slate-100 flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base font-black" style={{ color: "#000223" }}>
          ...
          <div className="px-6 py-6 flex flex-col gap-4">
            <div className="flex items-center justify-between">
              <div>
                <h3 className="font-bold text-sm text-[#000223]">
                  Connection Status
                </h3>
                <p className="text-xs text-slate-500 font-medium mt-1">
                  Connect your Google Workspace account to automatically sync
                  confirmed bookings.
                </p>
              </div>
              <div>
      ...
      <div className="mt-6 p-5 rounded-2xl bg-blue-50 border border-blue-100 flex items-start gap-3">
        ...
        <div>
          ...
          <p className="text-xs font-semibold text-blue-600 mt-0.5">
            All values are read directly from the database by the AI Engine on
            every booking request. No code redeployment needed.
          </p>
        </div>
      </div>
    </div>
