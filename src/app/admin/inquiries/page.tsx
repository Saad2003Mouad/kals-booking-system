...
export default function AdminInquiriesPage() {
  ...
  const updateInquiry = async (id: string, data: any) => {
    ...
    try {
      ...
      if (res.ok && json.success) {
        setInquiries((prev) =>
          prev.map((i) => (i.id === id ? { ...i, ...json.data } : i)),
        );
        if (selected?.id === id)
          setSelected((prev: any) => ({ ...prev, ...json.data }));
      }
    } catch (e) {}
    setUpdating("");
  };
  ...
  const sendEmailReply = async () => {
    ...
    try {
      const res = await fetch(`/api/admin/inquiries/${selected.id}/reply`, {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          subject: "Re: Your Inquiry with Boston Legend",
          body: replyBody,
        }),
      });
      const json = await res.json();
  ...
  if (error)
    return (
      <div className="p-10 text-red-500 font-bold text-center bg-red-50 rounded-xl">
        {error}
      </div>
    );
  ...
  return (
    <div className="flex h-[calc(100vh-80px)] overflow-hidden">
      {/* Main Content */}
      <div
        className={`flex-1 overflow-y-auto pr-6 transition-all ${selected ? "hidden lg:block lg:w-1/2" : "w-full"}`}
      >
        <div className="flex justify-between items-end mb-6">
          <div>
            <h1 className="text-3xl font-black text-[#000223]">Inquiries</h1>
            <p className="text-sm font-semibold text-slate-500 mt-1">
              Manage customer questions and escalations.
            </p>
          </div>
          <div className="flex items-center gap-3">
            ...
            <select
              ...
              className="input-premium py-2 max-w-[200px]"
            >
          ...
          <div className="p-4 border-b border-slate-100">
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-9 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm font-semibold outline-none focus:border-blue-500"
              />
            </div>
          </div>
            ...
            <div className="divide-y divide-slate-100">
              {filtered.map((i) => (
                <div
                  ...
                  onClick={() => {
                    setSelected(i);
                    setInternalNote(i.internalNote || "");
                  }}
                  className={`p-4 hover:bg-slate-50 cursor-pointer transition-colors flex flex-col sm:flex-row sm:items-center gap-4 ${selected?.id === i.id ? "bg-blue-50/50" : ""}`}
                >
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <span className="font-bold text-[#000223] truncate">
                        {i.name}
                      </span>
                      <span
                        className={`text-[10px] font-black uppercase px-2 py-0.5 rounded-md ${STATUS_COLORS[i.status]}`}
                      >
                        {i.status.replace("_", " ")}
                      </span>
                      <span
                        className={`text-[10px] font-black uppercase px-2 py-0.5 rounded-md ${PRIORITY_COLORS[i.priority]}`}
                      >
                        {i.priority}
                      </span>
                    </div>
                    <div className="text-xs font-semibold text-slate-500 flex items-center gap-3">
                      ...
                      {i.assignedTo && (
                        <span className="flex items-center gap-1 text-blue-600">
                          <Users className="w-3 h-3" /> {i.assignedTo.name}
                        </span>
                      )}
                    </div>
                    {i.notes && (
                      <p className="text-sm font-medium text-slate-600 mt-2 line-clamp-1">
                        {i.notes}
                      </p>
                    )}
                  </div>
                  <div className="shrink-0 hidden sm:block text-blue-500">
      ...
      {selected && (
        <div className="w-full lg:w-[500px] h-full bg-white border-l border-slate-200 shadow-2xl flex flex-col absolute right-0 top-0 lg:relative z-10 animate-in slide-in-from-right duration-300">
          <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
            <h2 className="font-black text-lg text-[#000223]">
              Inquiry Details
            </h2>
            <button
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-6">
            <div className="flex gap-2">
              ...
              <select
                ...
                onChange={(e) =>
                  updateInquiry(selected.id, { priority: e.target.value })
                }
                disabled={!!updating}
                className="input-premium py-1.5 text-xs"
              >
            ...
            <div className="space-y-3">
              ...
              <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 space-y-2 text-sm font-semibold">
                <div className="flex justify-between">
                  ...
                  <span className="text-[#000223]">{selected.name}</span>
                </div>
                <div className="flex justify-between">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="text-slate-800">{selected.source}</span>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-3">
              ...
              <textarea
                ...
                onBlur={() => {
                  if (internalNote !== selected.internalNote)
                    updateInquiry(selected.id, { internalNote });
                }}
              ...
              {showTaskForm && (
                <div className="flex gap-2 animate-in slide-in-from-top-2">
                  <input
                    ...
                    className="input-premium py-1.5 flex-1 text-sm"
                  />
                  <button
                    onClick={createTask}
                    disabled={updating === "task"}
              ...
              {selected.tasks?.length > 0 ? (
                <div className="space-y-2">
                  {selected.tasks.map((t: any) => (
                    <div
                      key={t.id}
                      className="flex justify-between items-center p-3 bg-white border border-slate-200 rounded-lg text-sm font-semibold"
                    >
