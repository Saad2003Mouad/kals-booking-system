  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [resTasks, resStaff] = await Promise.all([
        fetch(
          `/api/admin/tasks${statusFilter !== "ALL" ? `?status=${statusFilter}` : ""}`,
        ),
  ...
  return (
    <div className="space-y-6 pb-10">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <button
          onClick={openAdd}
          className="btn-primary py-2.5 px-5 text-sm flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 border-indigo-600"
        >
        ...
        <div className="p-4 border-b border-slate-100 flex gap-4 flex-wrap">
          <div className="relative flex-1 min-w-[200px]">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm font-semibold outline-none focus:border-indigo-500"
            />
          </div>
          <select
            value={statusFilter}
        ...
        {loading ? (
          ...
          <div className="divide-y divide-slate-100">
            {filtered.map((t) => (
              <div
                key={t.id}
                className="p-5 hover:bg-slate-50 transition-colors flex flex-col sm:flex-row gap-4"
              >
                <div className="pt-1">
                  <input
                    type="checkbox"
                    ...
                    onChange={(e) =>
                      updateStatusFast(t.id, e.target.checked ? "DONE" : "TODO")
                    }
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <span
                      className={`font-black text-[15px] truncate ${t.status === "DONE" ? "text-slate-400 line-through" : "text-[#000223]"}`}
                    >
                      {t.title}
                    </span>
                    <span
                      className={`text-[10px] font-black uppercase px-2 py-0.5 rounded-md ${STATUS_COLORS[t.status]}`}
                    >
                      {t.status.replace("_", " ")}
                    </span>
                    <span
                      className={`text-[10px] font-black uppercase px-2 py-0.5 rounded-md ${PRIORITY_COLORS[t.priority]}`}
                    >
                      {t.priority}
                    </span>
                  </div>
                  ...
                  <div className="flex flex-wrap items-center gap-4 text-xs font-bold text-slate-500">
                    ...
                    {t.inquiry && (
                      <span className="flex items-center gap-1.5 bg-amber-50 text-amber-700 px-2 py-0.5 rounded border border-amber-100">
                        Related Inquiry: {t.inquiry.name}
                      </span>
                    )}
                  </div>
                </div>
                <div className="flex sm:flex-col gap-2 shrink-0">
                  <button
                    onClick={() => openEdit(t)}
                    className="text-slate-400 hover:text-blue-600 p-2 bg-white border border-slate-200 rounded-lg shadow-sm"
                  >
                    <Edit className="w-4 h-4" />
                  </button>
                  <button
                    onClick={() => deleteTask(t.id)}
                    className="text-slate-400 hover:text-red-600 p-2 bg-white border border-slate-200 rounded-lg shadow-sm"
                  >
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4">
          <div className="bg-white rounded-3xl shadow-xl w-full max-w-xl overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <form
              onSubmit={handleSave}
              className="overflow-y-auto p-6 flex-1 space-y-4"
            >
              <div>
              ...
              <div>
                ...
                <textarea
                  ...
                  className="input-premium py-2.5 min-h-[100px]"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    ...
                    className="input-premium py-2.5 font-bold"
                  >
                    <option value="TODO">To Do</option>
                    <option value="IN_PROGRESS">In Progress</option>
                    <option value="DONE">Done</option>
            ...
            <div className="px-6 py-4 bg-slate-50 border-t border-slate-100 flex justify-end gap-3">
              <button
                type="button"
                onClick={() => setShowModal(false)}
                className="btn-secondary py-2 px-6 border-slate-200"
              >
                Cancel
              </button>
              <button
                onClick={handleSave}
                disabled={saving}
                className="btn-primary py-2 px-8 flex items-center gap-2 bg-indigo-600 border-indigo-600 hover:bg-indigo-700"
              >
                {saving ? (
