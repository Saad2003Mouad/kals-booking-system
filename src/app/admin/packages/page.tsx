...
const EMPTY_PACKAGE = {
  ...
  sortOrder: 10,
  image:
    "https://cdn.prod.website-files.com/67dc601bc29781a5af1632a2/67dc601bc29781a5af163351_image-01-products-boston-legend-ice-cream-truck.webp",
  isActive: true,
  features: [],
};
...
function PremiumImageUploader({
  ...
  return (
    <div className="space-y-4">
      <label className="block text-xs font-black uppercase tracking-widest text-[#000223]">
        Package Image
      </label>
        ...
        {uploading ? (
          ...
          <div className="space-y-2">
            ...
            <div className="text-xs font-semibold text-slate-550">
              ...
              <label className="text-[#FFA000] hover:underline cursor-pointer font-black">
                choose a file
                <input
                  type="file"
      ...
      {localPreview && localPreview.startsWith("https://") && (
        <div className="bg-emerald-50/80 border border-emerald-200 p-3.5 rounded-xl text-[11px] text-emerald-800 font-bold flex items-start gap-2 leading-relaxed">
          ...
          <span>
            <strong>Success!</strong> Image is uploaded to secure production
            storage.
          </span>
        </div>
      )}
      ...
      <div className="border border-slate-200/80 rounded-xl overflow-hidden bg-white shadow-sm">
        ...
        >
          ...
          <span className="text-[10px] font-black text-[#FFA000]">
        ...
        {showAdvanced && (
          <div className="p-4 bg-white">
            ...
            <p className="text-[9.5px] font-semibold text-slate-400 mt-1.5">
              Provide a web address to save this image permanently in the
              database.
            </p>
          </div>
        )}
...
export default function AdminPackagesPage() {
  ...
  const toggleStatus = async (pkg: Package) => {
    if (
      !confirm(
        `Are you sure you want to ${pkg.isActive ? "DEACTIVATE" : "ACTIVATE"} ${pkg.name}?`,
      )
    )
      return;
    try {
  ...
  const deletePackage = async (pkg: Package) => {
    if (
      !confirm(
        `WARNING: Are you sure you want to PERMANENTLY DELETE ${pkg.name}?`,
      )
    )
      return;
    try {
      ...
      <div className="flex justify-between items-end mb-8">
        <div>
          ...
          <p className="text-slate-500 font-semibold mt-1 text-sm">
            Add, edit, or disable Boston Legend packages. Changes sync
            instantly.
          </p>
        </div>
        <button
      ...
      {loading ? (
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          {packages.map((pkg) => (
            <div
              key={pkg.id}
              className={`bg-white border rounded-2xl p-6 shadow-sm transition-all duration-300 hover:shadow-md hover:-translate-y-0.5 ${pkg.isActive ? "border-slate-200" : "border-slate-200 opacity-60 bg-slate-50/50"}`}
            >
              <div className="relative h-44 w-full rounded-3xl overflow-hidden mb-5 bg-slate-50 border border-slate-100">
                ...
                <div className="absolute top-3 right-3 flex gap-1.5">
                  <button
                    ...
                    className="p-2.5 bg-white/95 hover:bg-white rounded-xl text-slate-700 hover:text-blue-600 shadow-md transition-all hover:scale-105"
                  >
                    <Edit2 className="w-4 h-4" />
                  </button>
                  <button
                    onClick={() => deletePackage(pkg)}
                    className="p-2.5 bg-white/95 hover:bg-white rounded-xl text-slate-700 hover:text-red-600 shadow-md transition-all hover:scale-105"
                  >
              ...
              <div className="flex justify-between items-start mb-2">
                <div className="font-black text-xl text-[#000223] leading-tight">
                  {pkg.name}
                </div>
              </div>
              ...
              <div className="flex items-center gap-2 mb-4">
                <span className="text-2xl font-black text-[#FFA000]">
                  ${(pkg.price || pkg.basePrice || 0).toFixed(2)}
                </span>
              ...
              <div className="space-y-2.5 text-sm text-[#000223]/70 font-bold mb-5 bg-[#FAF6EF] p-4.5 rounded-2xl border border-[#FAF6EF]/80">
                <div className="flex justify-between">
                  ...
                  <span className="font-black text-[#000223]">
                    {pkg.servings || pkg.includedQty}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span>Extra Guest:</span>{" "}
                  <span className="font-black text-[#000223]">
                    ${pkg.extraGuestPrice ?? pkg.extraPiecePrice}/person
                  </span>
                </div>
                <div className="flex justify-between">
                  <span>Duration:</span>{" "}
                  <span className="font-black text-[#000223]">
                    {pkg.durationMins ?? 60} min
                  </span>
                </div>
                <div className="flex justify-between">
              ...
              <div className="flex items-center justify-between pt-4 border-t border-slate-100">
                <div className="flex items-center gap-2">
                  {pkg.isActive ? (
                    <>
                      ...
                      <span className="text-xs font-black tracking-wide text-emerald-600">
                        ACTIVE
                      </span>
                    </>
                  ) : (
                    <>
                      ...
                      <span className="text-xs font-black tracking-wide text-slate-500">
                        DISABLED
                      </span>
                    </>
                  )}
                ...
                <button
                  ...
                  className="text-xs font-black text-[#000223]/60 hover:text-blue-600 transition-colors uppercase tracking-wider"
                >
                  Toggle Status
                </button>
              </div>
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-3xl overflow-hidden flex flex-col max-h-[90vh] animate-in fade-in zoom-in duration-200 border border-slate-200">
            <div className="px-8 py-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
              <h2 className="font-black text-xl text-[#000223] flex items-center gap-2">
                <PackageIcon className="w-5 h-5 text-[#FFA000]" />{" "}
            ...
            <form
              onSubmit={handleSave}
              className="overflow-y-auto p-8 flex-1 bg-white space-y-6"
            >
              <div className="grid md:grid-cols-2 gap-6">
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full py-3.5 px-5 rounded-2xl border-2 font-semibold text-base outline-none transition-all bg-white text-[#000223] border-slate-100 placeholder:text-slate-400 focus:border-[#FFA000] focus:ring-4 focus:ring-[#FFA000]/15"
                      placeholder="E.g. The Legend Classic"
                    />
                  </div>
                  <div>
                    ...
                    <textarea
                      ...
                      className="w-full py-3.5 px-5 rounded-2xl border-2 font-semibold text-base outline-none transition-all bg-white text-[#000223] border-slate-100 placeholder:text-slate-400 focus:border-[#FFA000] focus:ring-4 focus:ring-[#FFA000]/15 h-28"
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full py-3.5 px-5 rounded-2xl border-2 font-black text-base outline-none transition-all bg-white text-[#FFA000] border-slate-100 focus:border-[#FFA000] focus:ring-4 focus:ring-[#FFA000]/15"
                      />
                    </div>
                    <div>
                      ...
                      <input
                        ...
                        className="w-full py-3.5 px-5 rounded-2xl border-2 font-semibold text-base outline-none transition-all bg-white text-[#000223] border-slate-100 focus:border-[#FFA000] focus:ring-4 focus:ring-[#FFA000]/15"
                      />
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <input
                        ...
                        step="0.01"
                        value={
                          formData.extraGuestPrice ?? formData.extraPiecePrice
                        }
                        onChange={(e) =>
                          setFormData({
                            ...formData,
                            extraGuestPrice: e.target.value,
                            extraPiecePrice: e.target.value,
                          })
                        }
                        className="w-full py-3.5 px-5 rounded-2xl border-2 font-semibold text-base outline-none transition-all bg-white text-[#000223] border-slate-100 focus:border-[#FFA000] focus:ring-4 focus:ring-[#FFA000]/15"
                      />
                ...
                <div className="md:col-span-2">
                  ...
                  <p className="text-xs font-semibold text-slate-400 mt-1.5">
                    These will appear as bullet points under the package.
                  </p>
                </div>
              </div>
            </form>
            ...
            <div className="px-8 py-5 bg-slate-50 border-t border-slate-100 flex justify-end gap-3">
              <button
                type="button"
                onClick={() => setShowModal(false)}
                className="btn-secondary py-3 px-6 rounded-2xl font-extrabold text-sm border-2 border-slate-200/80 bg-white text-[#000223] transition-all hover:bg-slate-50"
              >
                Cancel
              </button>
              <button
                onClick={handleSave}
                disabled={saving}
                className="btn-primary py-3 px-8 rounded-2xl font-black text-sm text-[#000223] bg-[#FFA000] hover:bg-[#FFB020] hover:shadow-lg disabled:opacity-50 transition-all flex items-center gap-2"
              >
                {saving ? (
