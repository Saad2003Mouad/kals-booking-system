...
> = {
  PENDING_REVIEW: {
    ...
    text: "text-amber-800",
    icon: Clock,
    desc: "Our team is reviewing your event details and will prepare your custom quote. We'll reach out via WhatsApp!",
  },
  PENDING_PAYMENT: {
    ...
    text: "text-blue-800",
    icon: DollarSign,
    desc: "Your booking is approved! We are finalizing the details.",
  },
  CONFIRMED: {
    label: "Confirmed",
    bg: "bg-emerald-100",
    ...
    icon: CheckCircle2,
    desc: "Awesome! Your legendary ice cream event is fully confirmed.",
  },
  COMPLETED: {
    label: "Completed",
    bg: "bg-slate-100",
    ...
    text: "text-slate-800",
    icon: CheckCircle2,
    desc: "This event has been completed. Thank you for choosing Boston Legend!",
  },
  CANCELLED: {
    label: "Cancelled",
    bg: "bg-red-100",
    ...
    text: "text-red-800",
    icon: XCircle,
    desc: "This booking request has been cancelled.",
  },
  REJECTED: {
    label: "Needs Info / Rejected",
    bg: "bg-rose-100",
    border: "border-rose-300",
    text: "text-rose-800",
    icon: XCircle,
    desc: "This request requires updates or has been declined.",
  },
};

export default function CustomerBookingPortal({
  params,
}: {
  params: { token: string };
}) {
  ...
  const loadBooking = async () => {
    ...
    try {
      ...
      if (res.ok && json.success) {
        ...
        setError(
          json.error ||
            "We couldn't retrieve your booking details. Please verify your link.",
        );
      }
    } catch (e) {
  ...
  const handleRequestAction = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok && json.success) {
        setSubmitSuccess(
          `Your ${showRequestModal === "CANCEL" ? "cancellation" : "modification"} request was submitted to the operations team.`,
        );
        setRequestReason("");
        setTimeout(() => {
        ...
        <div className="text-center animate-in fade-in zoom-in duration-500">
          <div className="relative">
            ...
            <img
              src={LOGO}
              alt="Boston Legend"
              className="h-20 w-auto relative z-10 mx-auto mb-6"
            />
          </div>
          <Loader2 className="w-10 h-10 animate-spin mx-auto text-[#FFA000]" />
          <p className="mt-4 font-black text-[#000223] tracking-widest uppercase text-xs">
            Loading Portal
          </p>
        </div>
      </div>
      ...
      <div className="min-h-screen bg-[#FAF6EF] flex flex-col justify-between font-['Nunito',sans-serif]">
        <header className="bg-[#000223] py-4 px-6 border-b border-white/10 shadow-lg">
          <div className="max-w-6xl mx-auto flex items-center justify-between">
            ...
            <a
              href="/packages"
              className="px-5 py-2.5 rounded-full font-black text-sm bg-[#FFA000] text-[#000223] hover:bg-white transition-all shadow-md"
            >
              Start Your Booking
            </a>
          </div>
        ...
        <main className="flex-grow flex items-center justify-center px-4 py-16">
          <div className="max-w-xl w-full bg-white rounded-[32px] p-10 border border-slate-100 shadow-[0_20px_50px_rgba(0,2,35,0.06)] text-center relative overflow-hidden">
            ...
            <h2 className="text-2xl font-black text-[#000223] mb-4">
              Booking Not Found
            </h2>
            <p className="text-slate-500 font-bold leading-relaxed mb-8">
              We couldn’t find this booking. The link might have expired or the
              booking number is incorrect.
            </p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <a
                href="/packages"
                className="py-3 px-8 rounded-full font-black text-sm text-[#000223] bg-[#FFA000] hover:bg-[#FFB020] transition-all shadow-md"
              >
                Start New Booking
              </a>
              <a
                href="/contact-us"
                className="py-3 px-8 rounded-full font-black text-sm text-white bg-[#000223] hover:bg-slate-800 transition-all shadow-md"
              >
                Contact Support
              </a>
            </div>
  ...
  const extraGuestsCount =
    breakdown.additionalGuests ?? Math.max(0, booking.guests - servingsLimit);
  ...
  const billableMiles =
    breakdown.billableMiles ?? Math.max(0, totalMiles - freeMiles);
  const travelFee = breakdown.travelFee ?? quote?.travelFee ?? 0;
  const includedServiceMins =
    breakdown.includedServiceMins ??
    pkg?.durationMins ??
    pkg?.includedMinutes ??
    booking.durationMins;
  const extraServiceMins =
    breakdown.additionalServiceMins ?? (booking.extraServiceMins || 0);
  const extraServiceFee =
    breakdown.additionalServiceFee ??
    (booking.extraServiceFee || (extraServiceMins / 30) * 35);
  const packageName =
    breakdown.packageName ?? (pkg?.name || "Ice Cream Truck Booking");
  ...
  const additionalLocationServiceFee =
    breakdown.additionalLocationServiceFee ?? stopsFee;
  const isCustomPkg =
    pkg?.slug === "custom-event-package" ||
    (pkg as any)?.serviceType === "CUSTOM";
  ...
  const getWhatsAppUrl = (waPhone: string) => {
    ...
    const stopsList =
      booking.stops && booking.stops.length > 0
        ? booking.stops
            .map((s: any, i: number) => `Stop ${i + 2}: ${s.street}, ${s.city}`)
            .join(", ")
        : "None";
    const travelDist = quote?.distanceMiles
      ? `${quote.distanceMiles.toFixed(1)} miles`
      : "N/A";
    ...
    <div className="min-h-screen bg-[#FAF6EF] pb-24 font-['Nunito',sans-serif] selection:bg-[#FFA000] selection:text-[#000223] relative overflow-x-hidden">
      ...
      <div className="absolute top-0 left-0 right-0 h-20 z-20 pointer-events-none overflow-hidden print:hidden">
        ...
        <div
          ...
          style={{
            background:
              "linear-gradient(105deg, #FF6B00 0%, #FF8C00 45%, #FFA500 70%, transparent 100%)",
            opacity: 0.75,
        ...
        <div
          ...
          style={{
            background:
              "linear-gradient(255deg, #C2185B 0%, #E91E8C 45%, #FF4DB2 70%, transparent 100%)",
            opacity: 0.75,
      ...
      <header className="relative z-30 print:hidden h-20 flex items-center">
        <div className="w-full max-w-6xl mx-auto px-6 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <a
              href="/packages"
              className="px-5 py-2.5 rounded-full font-black text-xs bg-[#000223] text-white hover:bg-[#FFA000] hover:text-[#000223] transition-all"
            >
              Book Another Event
            </a>
          </div>
        ...
        <div className="max-w-4xl mx-auto flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="flex-1">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-slate-100 border border-slate-200 mb-4">
              ...
              <span className="text-xs font-mono font-black text-[#000223]">
          ...
          <div className="flex flex-col items-start md:items-end gap-3 shrink-0">
            <div
              className={`inline-flex items-center gap-2 px-4 py-2 rounded-xl border ${sc.bg} ${sc.border} shadow-sm`}
            >
              ...
              <span
                className={`text-xs font-black uppercase tracking-wider ${sc.text}`}
              >
                {sc.label}
              </span>
            </div>
      ...
      <div className="max-w-4xl mx-auto px-6 mt-8 print:hidden mb-8">
        <div className="bg-white border border-slate-200 rounded-xl p-3 shadow-sm flex flex-wrap gap-3 justify-start md:justify-end">
          {["PENDING_REVIEW", "PENDING_PAYMENT", "CONFIRMED"].includes(
            booking.status,
          ) && (
            <>
              <button
                onClick={() => setShowRequestModal("CHANGE")}
                className="flex-1 sm:flex-initial py-2.5 px-4 bg-slate-50 hover:bg-slate-100 text-[#000223] rounded-lg font-bold text-xs transition-all flex items-center justify-center gap-2 border border-slate-200"
              >
                <Edit className="w-3.5 h-3.5 text-[#000223]" /> Request Change
              </button>
              <button
                onClick={() => setShowRequestModal("CANCEL")}
                className="flex-1 sm:flex-initial py-2.5 px-4 bg-rose-50 hover:bg-rose-100 text-rose-700 rounded-lg font-bold text-xs transition-all flex items-center justify-center gap-2 border border-rose-200"
              >
          ...
          {["CONFIRMED", "PENDING_PAYMENT"].includes(booking.status) && (
            <>
              <a
                ...
                className="flex-1 sm:flex-initial py-2.5 px-4 bg-blue-50 hover:bg-blue-100 text-blue-700 rounded-lg font-bold text-xs transition-all flex items-center justify-center gap-2 border border-blue-200"
              >
                <Calendar className="w-3.5 h-3.5" /> Add to Google
              </a>
              <a
                href={`/api/customer/bookings/${booking.id}/calendar`}
                className="flex-1 sm:flex-initial py-2.5 px-4 bg-slate-50 hover:bg-slate-100 text-slate-700 rounded-lg font-bold text-xs transition-all flex items-center justify-center gap-2 border border-slate-200"
              >
          ...
          <button
            ...
            className="flex-1 sm:flex-initial py-2.5 px-4 bg-[#000223] hover:bg-slate-800 text-white rounded-lg font-bold text-xs transition-all flex items-center justify-center gap-2 shadow-sm"
          >
      ...
      <div className="max-w-4xl mx-auto px-6 grid md:grid-cols-2 gap-6 relative z-10">
        ...
        <div className="bg-white rounded-xl p-6 border border-slate-200 shadow-[0_2px_8px_rgba(0,0,0,0.02)]">
          ...
          <div className="space-y-4 text-sm font-bold text-slate-500">
            <div className="flex justify-between items-center">
              <span>Date</span>
              <span className="font-black text-[#000223] bg-slate-50 px-3 py-1 rounded-lg">
                {eventDate}
              </span>
            </div>
            <div className="flex justify-between items-center">
            ...
            <div className="flex justify-between items-center">
              ...
              <span className="font-black text-[#000223]">
                {includedServiceMins} mins
              </span>
            </div>
            {extraServiceMins > 0 && (
              <div className="flex justify-between items-center text-amber-600">
                <span>Extra Time</span>
                <span className="font-black bg-amber-50 px-2 py-0.5 rounded">
                  +{extraServiceMins} mins (+${extraServiceFee.toFixed(2)})
                </span>
              </div>
            )}
            <div className="flex justify-between items-center pt-2 border-t border-slate-100">
              <span>Expected Guests</span>
              <span className="font-black text-[#000223] bg-amber-100 px-3 py-1 rounded-lg">
        ...
        <div className="bg-white rounded-xl p-6 border border-slate-200 shadow-[0_2px_8px_rgba(0,0,0,0.02)]">
          ...
          <div className="space-y-4 text-sm font-bold text-slate-500">
            <div className="flex justify-between items-center">
              <span>Package</span>
              <span className="font-black text-[#000223] text-right truncate max-w-[180px]">
                {packageName}
              </span>
            </div>
            <div className="flex justify-between items-center">
            ...
            {stopsCount > 0 && (
              <div className="flex justify-between items-center text-emerald-600">
                ...
                <span className="font-black bg-emerald-50 px-2 py-0.5 rounded">
                  +${stopsFee.toFixed(2)}
                </span>
              </div>
            )}
          ...
          <div className="grid md:grid-cols-2 gap-8">
            <div className="space-y-4 text-sm font-bold text-slate-500">
              <div className="flex flex-col">
                ...
                <span className="font-black text-[#000223] text-base">
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="text-[10px] font-black uppercase tracking-wider text-slate-400 mb-1">
                    Primary Setup Location
                  </p>
                  <p className="font-black text-[#000223] leading-snug">
                    {booking.address}
                  </p>
                  <p className="font-bold text-slate-500 text-xs mt-0.5">
                    {booking.city}, MA {booking.zip}
                  </p>
                  {booking.notes && (
                    <div className="mt-3 pt-3 border-t border-slate-200">
                      ...
                      <p className="text-xs font-semibold text-slate-650 italic">
                        "{booking.notes}"
                      </p>
                    </div>
                  )}
            ...
            <div className="space-y-4">
              <div className="flex items-start gap-4 p-4 bg-slate-50 border border-slate-200 rounded-xl">
                ...
                <div>
                  ...
                  <p className="font-black text-[#000223] text-sm mt-0.5">
                    {booking.address}
                  </p>
              ...
              {booking.stops.map((stop: any, idx: number) => (
                ...
                >
                  ...
                  <div>
                    ...
                    {stop.notes && (
                      <p className="text-xs font-semibold text-slate-555 mt-2 italic bg-slate-50 p-2 rounded-lg">
                        "{stop.notes}"
                      </p>
                    )}
                  </div>
        ...
        <div className="bg-[#000223] rounded-xl p-6 border border-[#000223] text-white flex flex-col justify-between">
          <div>
            ...
            <div className="flex justify-between items-end border-b border-white/10 pb-4 mb-4">
              ...
              {isCustomPkg && booking.status === "PENDING_REVIEW" ? (
                <span className="text-2xl font-black text-[#FFA000]">
                  Custom Quote
                </span>
              ) : (
                <span className="text-4xl font-black text-[#FFA000] tracking-tight">
            ...
            {!isCustomPkg &&
              (additionalVehicleSetupFee > 0 || weekendFee > 0) && (
                <div className="mt-4 space-y-2 border-t border-white/10 pt-4 text-left">
                  {additionalVehicleSetupFee > 0 && (
                    <p className="text-[11px] font-bold text-white/80 leading-relaxed">
                      🚚 <strong>Additional Vehicle Setup Fee:</strong> If your
                      event requires another truck/van for the same package at
                      the same time, each additional vehicle includes a $200
                      setup and dispatch fee.
                    </p>
                  )}
                  {weekendFee > 0 && (
                    <p className="text-[11px] font-bold text-white/80 leading-relaxed">
                      📅 <strong>Weekend Event Fee:</strong> Saturday and Sunday
                      bookings include an additional $25 weekend event fee.
                    </p>
                  )}
                </div>
              )}
          ...
          {isCustomPkg && booking.status === "PENDING_REVIEW" ? (
            <div className="space-y-2 mt-4">
              <p className="text-xs font-bold text-white/80 mb-2 leading-relaxed">
                Our team will reach out via WhatsApp to finalize your custom
                quote:
              </p>
              {[
            ...
            <div className="bg-white/10 rounded-lg p-4 border border-white/10 mt-4">
              <p className="text-xs font-bold text-white/80 leading-relaxed">
                {breakdown.paymentPolicy ||
                  "Payment is collected after the service is successfully provided."}
              </p>
            ...
            <form onSubmit={handleUpdateContact} className="space-y-4">
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  className="w-full py-3.5 px-4 rounded-xl border-2 font-bold text-sm outline-none transition-all bg-slate-50 text-[#000223] border-slate-200 placeholder:text-slate-400 focus:border-[#000223] focus:bg-white"
                />
              </div>
              <div className="space-y-1.5">
                <label className="text-[11px] font-black uppercase tracking-widest text-slate-500">
              ...
              <div className="space-y-1.5">
                ...
                <textarea
                  ...
                  className="w-full py-3.5 px-4 rounded-xl border-2 font-bold text-sm outline-none transition-all bg-slate-50 text-[#000223] border-slate-200 placeholder:text-slate-400 focus:border-[#000223] focus:bg-white"
                  rows={3}
                  placeholder="Add gate details or special requests..."
                />
              </div>
              ...
              <div className="pt-4">
                <button
                  type="submit"
                  disabled={submitting}
                  className="w-full py-3.5 rounded-xl font-bold text-sm text-[#000223] bg-[#FFA000] hover:bg-[#e69000] transition-colors flex items-center justify-center gap-2 shadow-sm"
                >
                  {submitting ? (
                    <>
            ...
            <form onSubmit={handleRequestAction} className="space-y-4">
                ...
                <textarea
                  ...
                  placeholder={
                    showRequestModal === "CANCEL"
                      ...
                      : "E.g. Change time from 2:00 PM to 4:00 PM..."
                  }
                />
              </div>
              ...
              <div className="pt-4">
                <button
                  ...
                  className="w-full py-3.5 rounded-xl font-bold text-sm text-white bg-[#000223] hover:bg-[#FFA000] hover:text-[#000223] transition-all flex items-center justify-center gap-2 shadow-sm disabled:opacity-70 disabled:cursor-not-allowed"
                >
                  {submitting ? (
                    <>
